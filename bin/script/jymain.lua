...
function Game_MMap()      --主地图
    ...
    if CC.ShowXY==1 then
	    DrawString(10,CC.Height-20,string.format("%d %d",JY.Base["人X"],JY.Base["人Y"]) ,C_GOLD,16);
	end
    ShowScreen();
    ...
    if JY.SubScene >= 0 then          --进入子场景
        ...
        lib.ShowSlow(50,0)
        lib.GetKey();
		DrawStrBoxWaitKey(JY.Scene[JY.SubScene]["名称"],C_WHITE,CC.DefaultFont);
    end
end
...
function Game_SMap()         --场景处理主函数
    ...
    DrawSMap();
	if CC.ShowXY==1 then
        DrawString(10,CC.Height-20,string.format("%s %d %d",JY.Scene[JY.SubScene]["名称"],JY.Base["人X1"],JY.Base["人Y1"]) ,C_GOLD,16);
	end
    ShowScreen();
...
--key  访问的key
function GetDataFromStruct(data,t_struct,key)  --从数据的结构中翻译数据，用来取数据
...
function SetDataFromStruct(data,t_struct,key,v)  --从数据的结构中翻译数据，保存数据
    local t=t_struct[key];
	if t[2]==0 then
...
function UseThing_Type1(id)            --装备物品使用
    ...
    if r>0 then
        ...
        if CanUseThing(id,personid) then
            ...
            Cls();
            DrawStrBox(-1,-1,"此人不适合配备此物品",C_WHITE,CC.DefaultFont);
            ShowScreen();
            WaitKey();
			return 0;
        end
    ...
    if r>0 then
        ...
        if JY.Thing[id]["练出武功"]>=0 then
            ...
            if yes==0 and JY.Person[personid]["武功10"]>0 then
                Cls();
                DrawStrBox(-1,-1,"一个人只能修炼10种武功",C_WHITE,CC.DefaultFont);
                ShowScreen();
                WaitKey();
                Cls();
                ShowScreen();
       ...
       if id==78 or id==93 then                --辟邪和葵花
			...
			elseif JY.Person[personid]["性别"]==1 then
				Cls();
				DrawStrBox(-1,-1,"此人不适合修炼此物品",C_WHITE,CC.DefaultFont);
				ShowScreen();
				WaitKey();
				return 0;
			end
...
function War_ShowFight(pid,wugong,wugongtype,eft)              --显示战斗动画
    ...
    for i=0,framenum-1 do
        ...
        if(WAR.Person[WAR.CurID]["贴图类型"]==0) then
            WarDrawMap(4,WAR.Person[WAR.CurID]["贴图"],0);
		else
            WarDrawMap(4,WAR.Person[WAR.CurID]["贴图"],4);
		end
    ...
    for i = 0, WAR.PersonNum-1 do
		...
		if WAR.Person[i]["死亡"]==false then
 		    if GetWarMap(x1,y1,4)>1 then
		        SetWarMap(x1,y1,3,WAR.Person[i]["点数"]);
 		    end
		end
	end
